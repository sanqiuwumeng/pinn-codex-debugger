...
This directory contains the credential-free, reproducible entry points for the
approved AutoDL end-to-end qualification.

1. `build_remote_inputs.py` reads the governed cross-domain knowledge
   candidate, verifies every referenced artifact, copies only those evidence
   files, and rewrites their URIs for the declared remote input root.
