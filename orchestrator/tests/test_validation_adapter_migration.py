...
SMOKE_ROOT = REPOSITORY_ROOT / "validation" / "smoke"
...
class ValidationAdapterMigrationTests(unittest.TestCase):
    def test_smoke_and_full_have_no_validation_only_process_backend(self) -> None:
        paths = (
            REPOSITORY_ROOT / "validation" / "smoke" / "run_governed_pinn_smoke.py",
            REPOSITORY_ROOT / "validation" / "full" / "run_governed_pinn_full.py",
        )
        for path in paths:
            ...
            self.assertNotIn("LocalObservedBackend", source)
            self.assertFalse(
                any(
        ...
        smoke_source = paths[0].read_text(encoding="utf-8")
        self.assertIn("runner.reconcile", smoke_source)
        self.assertIn("runner.collect", smoke_source)
