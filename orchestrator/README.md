...
7. `wiki publish` is a separate human-approved action. It verifies every
   candidate evidence hash and atomically appends one immutable Wiki version
   containing JSON, Markdown, a RAG index document and a manifest. It never
   promotes a Skill.
...
pinn-strategy wiki publish --candidate ... --approval ... --publication ... --knowledge-root ...
```
