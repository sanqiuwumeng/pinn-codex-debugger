"""Hybrid retrieval ranking regression tests."""
...
from hybrid_rag import HybridQuery, HybridSectionIndex, build_concept_lexicon
from rules_mcp import HandbookIndex

from .support import create_hybrid_service, expected_handbook_sha256


class HybridRetrievalTests(unittest.TestCase):
    def setUp(self) -> None:
        self.service = create_hybrid_service()
    ...
    def test_unseen_boundary_paraphrase_retrieves_boundary_section(self) -> None:
        ...
        self.assertTrue(
            any(section.matched_concepts for section in response.ranked_sections)
        )
    ...
    def test_repeated_ranking_is_identical(self) -> None:
        ...
        first = self.service.search(request).to_dict()
        second = self.service.search(request).to_dict()

        self.assertEqual(first, second)

    def test_ranked_sections_preserve_provenance(self) -> None:
        response = self.service.search(
            HybridQuery(query="局部亮斑残差一直压不下去。")
        )

        self.assertEqual(response.source_sha256, expected_handbook_sha256())
    ...
    def test_noisy_markdown_sections_are_indexed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            section_index = HybridSectionIndex.from_handbook(
                handbook_index, concepts
            )
