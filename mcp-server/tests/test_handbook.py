"""Handbook parser and exact-match extraction tests."""
...
class HandbookIndexTests(unittest.TestCase):
    ...
    def test_extracts_heading_ranges_from_noisy_markdown(self) -> None:
        chapter = next(
            item
            for item in self.index.sections
            if item.heading == "第十章 硬约束、距离函数与边界条件处理"
        )
        html_section = next(
            item for item in self.index.sections if "<table>" in item.text
        )
        ...
        self.assertIn("<table>", html_section.text)
