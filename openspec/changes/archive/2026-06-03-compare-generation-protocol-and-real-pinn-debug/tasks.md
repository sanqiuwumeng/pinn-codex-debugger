...
## 5. Code Debug Extension

- [x] 5.1 Copy `skill_test\PINN-2D` into three ignored, isolated workspaces without modifying the original directory.
- [x] 5.2 Authorize each scheme agent to make one logical code/configuration change and run smoke or full retraining only inside its own workspace.
- [x] 5.3 Preserve each agent's patch, command log summary, before/after metrics, and regression notes.
- [x] 5.4 Compare full-training results separately from smoke-only exploratory results.
- [x] 5.5 Report which scheme best assisted code debugging and identify the next engineering step.
